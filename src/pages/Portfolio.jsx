            ...
            <section className="py-10 pb-32 container mx-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {cases.map((project, index) => (
                        <Card key={index} className="p-8 md:p-12 hover:bg-[#1F1F1F] group cursor-pointer transition-all duration-300">
                            <div className="flex justify-between items-start mb-6">
                                <span className="font-mono text-xs text-[#00C896] uppercase tracking-wider border border-[#00C896]/30 px-3 py-1 rounded-full">
                                    {project.category}
                                </span>
                                <ArrowRight className="text-[#F1F1F1]/20 group-hover:text-[#00C896] group-hover:translate-x-2 transition-all duration-300" />
                            </div>
                            <h3 className="text-3xl font-['Sora'] font-bold mb-2 group-hover:text-[#00C896] transition-colors">{project.title}</h3>
                            <p className="text-[#F1F1F1]/50 text-sm mb-6">{project.client}</p>
